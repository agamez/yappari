...
#define ICON_PATH   "/usr/share/mitakuuluu3/emoji/%1x%1/"
...
Emoji *Emoji::GetInstance(QObject *parent)
{
    static Emoji* lsSingleton = NULL;
    if (!lsSingleton) {
        lsSingleton = new Emoji(parent);
    }
    return lsSingleton;
}

QString Emoji::waTextToHtml(QString data, int iconSize, bool urlConversion, const QString &linkColor)
{
    data.replace("&","&amp;");
    data.replace("<","&lt;");
    data.replace(">","&gt;");
    ...
    QString emojiPath = QString(ICON_PATH).arg(iconSize <= 32 ? 32 : 48);
    ...
    for (int i = 0; i < array.length(); i++)
    {
        ...
        if (c == 0xEE)
        {
            if (i < array.length() - 2)
            {
                ...
                if (emojiList.contains(code))
                {
                    QString emoji = makeEmojiHtml(emojiHeader, emojiPath, QString::number(e1,16), QString::number(e2 - 0x80));
        ...
        else if (c == 0xE2 || c== 0xE3)
        {
            if (i < array.length() - 2)
            {
                ...
                if (emojiList.contains(code))
                {
                    QString emoji = makeEmojiHtml(emojiHeader, emojiPath, QString::number(c,16), QString::number(e1,16) + QString::number(e2,16));
        ...
        else if (c == 0xF0)
        {
            if (i < array.length() - 3)
            {
                ...
                if (emojiList.contains(code))
                {
                    QString emoji = makeEmojiHtml(emojiHeader, emojiPath, QString::number(e1,16), QString::number(e2,16) + QString::number(e3,16));
        ...
        else if ((c >= 0x30 && c <= 0x39) || c == 0x23)
        {
            if (i < array.length() - 3)
            {
                ...
                if (e1 == 0xE2 && e2 == 0x83 && e3 == 0xA3)
                {
                    ...
                    QString emoji = makeEmojiHtml(emojiHeader, emojiPath, "88", QString::number(e));
        ...
        else if (c == 0xC2)
        {
            if (i < array.length() - 1)
            {
                ...
                QString emoji = makeEmojiHtml(emojiHeader, emojiPath, "89", QString::number(e));
    ...
    if (urlConversion)
    {
        ...
        if (!linkColor.isEmpty()) {
            result.prepend(QString("<style type=\"text/css\">a:link {color:%1;}</style>").arg(linkColor));
        }
    }
...
QString Emoji::htmlToWaText(QString html)
{
    //qDebug() << html;
...
QString Emoji::makeEmojiHtml(const QString &header, const QString &path, const QString &first, const QString &second)
{
    return QString("<img %1 src=\"%2%3-%4.png\" />").arg(header).arg(path).arg(first).arg(second);
}

QString Emoji::shortURL(QString url)
{
    if (url.size() > 40)
    {
        int charsToRemove = url.size() - 40;
        int pos = url.lastIndexOf("/");

        if (pos < charsToRemove + 6)
            pos = url.lastIndexOf(".");

        if (pos < charsToRemove + 6)
            pos = url.size() - 6;

        url = url.left(pos - (charsToRemove + 6)) + "..." + url.right(url.size() - pos);
    }
